...
| Codex CLI | `~/.codex/sessions/YYYY/MM/DD/rollout-<ts>-<uuid>.jsonl` | JSONL `{timestamp, type, payload}`; types `session_meta`, `response_item`, `event_msg`, `turn_context`, `compacted`, … | **analysis loader in progress** (`analysis/codex.py`); engine parser next |
