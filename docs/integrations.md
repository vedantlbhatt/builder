...
| Codex CLI | `~/.codex/sessions/YYYY/MM/DD/rollout-<ts>-<uuid>.jsonl` | JSONL `{timestamp, type, payload}`; types `session_meta`, `response_item`, `event_msg`, `turn_context`, `compacted`, … | **analysis loader shipping** (`analysis/codex.py`, probe available); engine parser next |
| Gemini CLI | `~/.gemini/tmp/<project_hash>/chats/*.json` (and `checkpoints/`) | JSON, whole conversation per file; role `user`/`model`, `parts[]` with `functionCall`/`functionResponse` | researched, not implemented |
