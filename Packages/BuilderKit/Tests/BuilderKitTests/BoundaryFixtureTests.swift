...
///
/// `scripts/measure_boundaries.py` is the reference implementation of the v2 rules and
/// `scripts/gen_boundary_fixtures.py` records what it says about eight synthetic
...
struct BoundaryFixtureTests {
    ...
    static func expected(for name: String) throws -> Expected {
        guard let dir = fixtureDirectory else {
    ...
    static func events(for name: String) throws -> [NormalizedEvent] {
        guard let dir = fixtureDirectory else { return [] }
        return try parse(jsonl: Data(contentsOf: dir.appendingPathComponent(name + ".jsonl")))
    }

    static func sessions(for name: String) throws -> [DetectedSession] {
        let options = Sessionizer.Options(pooling: .nativeSession, calendar: newYork)
        return Sessionizer.sessions(from: try events(for: name), options: options)
    }
